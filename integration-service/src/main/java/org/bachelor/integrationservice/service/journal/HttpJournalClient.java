...
public class HttpJournalClient implements JournalClient {
    ...
    @Override
    public List<JournalSpecialtyDTO> getSpecialties(String degree, Integer graduationYear, String studyForm, String code) {
        var uri = UriComponentsBuilder.fromUriString(baseUrl + "/specialties")
                ...
                .build().toUri();
        return restClient.get().uri(uri).retrieve().body(new ParameterizedTypeReference<List<JournalSpecialtyDTO>>() {});
    }
